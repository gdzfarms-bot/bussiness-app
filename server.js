...
// ==================== DATABASE CONNECTION ====================
const pool = new Pool({
    connectionString: "postgresql://neondb_owner:npg_KpT4ija0ENZb@ep-little-star-adsqx0be-pooler.c-2.us-east-1.aws.neon.tech/neondb?sslmode=require&channel_binding=require",
    ssl: {
...
app.post('/api/items', async (req, res) => {
    try {
        const { userId, name, quantity_value, quantity_unit, buying_price, selling_price } = req.body;
        const result = await pool.query(
        ...
        res.json({ success: true, item: result.rows[0], message: 'Item added successfully' });
    } catch (error) {
...
app.put('/api/items/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const { name, quantity_value, quantity_unit, buying_price, selling_price, userId } = req.body;
        const result = await pool.query(
            `UPDATE items SET name = $1, quantity_value = $2, quantity_unit = $3, 
             buying_price = $4, selling_price = $5, updated_at = NOW() 
             WHERE id = $6 AND user_id = $7 RETURNING *`,
            [name, quantity_value, quantity_unit, buying_price, selling_price, id, userId]
        );
...
app.delete('/api/items/:id/:userId', async (req, res) => {
    try {
        ...
        const result = await pool.query('DELETE FROM items WHERE id = $1 AND user_id = $2 RETURNING *', [id, userId]);
        if (result.rows.length === 0) return res.status(404).json({ success: false, message: 'Item not found' });
...
app.get('/api/analytics/:userId', async (req, res) => {
    try {
        ...
        const totalInvestment = items.reduce((sum, item) => sum + (item.buying_price * item.quantity_value), 0);
        const totalRevenue = items.reduce((sum, item) => sum + (item.selling_price * item.quantity_value), 0);
...
app.get('/api/settings/:userId', async (req, res) => {
    try {
        ...
        if (result.rows.length === 0) return res.status(404).json({ success: false, message: 'User settings not found' });
...
app.post('/api/goals', async (req, res) => {
    try {
        ...
        const result = await pool.query(
            `INSERT INTO goals (user_id, target_revenue, target_profit, deadline) VALUES ($1, $2, $3, $4) RETURNING *`,
            [userId, target_revenue, target_profit, deadline]
        );
        res.json({ success: true, goal: result.rows[0], message: 'Goal set successfully' });
    } catch (error) {
...
app.get('/api/goals/:userId', async (req, res) => {
    try {
        const { userId } = req.params;
        const result = await pool.query('SELECT * FROM goals WHERE user_id = $1 ORDER BY created_at DESC LIMIT 1', [userId]);
